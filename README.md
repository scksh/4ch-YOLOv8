...
# 4ch-YOLOv8
이 프로젝트에서 개발한 모델 **4ch-YOLOv8**은 GOP과학화 경계시스템과 군사 경계 체계에 활용 될 수 있도록 군인 및 민간인을 포함하는 클래스와 감시 환경에 자주 노출 되는 동물 6종으로 이루어진 클래스로 구성한 RGB + IR(열화상) 이미지로부터 4채널 데이터를 입력으로 사용하는 YOLOv8모델 기반의 객체 탐지 모델입니다.
